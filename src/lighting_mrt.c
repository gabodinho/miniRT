...
/*   Updated: 2024/08/22 12:25:07 by ggiertzu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	*get_diffuse(double *eff_col, double *light_v, t_comps *comps)
{
	...
	normalize(light_v);
	light_dot_norm = dot_product(light_v, comps -> normal_vec);
	if (light_dot_norm < 0)
		diffuse = point(0, 0, 0);
	else
	{
...
static double	*get_specular(double *light_v, t_comps *comps, t_world *w)
{
	...
	else
	{
		...
		reflect_v = reflect(light_v, comps -> normal_vec);
		if (dot_product(reflect_v, comps -> eye_vec) <= 0)
			specular = point(0, 0, 0);
		else
		{
			temp = w -> light_bright * SPECULAR
				* pow(dot_product(reflect_v, comps -> eye_vec), SHINE);
...
double	*lighting(t_world *w, t_comps *comps)
{
	...
	specular = get_specular(
		substract_points(w -> light_point, comps -> point_w), comps, w);
